...
void restart(void)
{
  ...
  //
  executable = (char *) "crystaleer.exe";
#endif
...
static std::string create_appdata_dir(void)
{
  ...
  char *dir = dynprintf("%s%s%s", appdata, DIR_SEP, "crystaleer");
#ifdef _WIN32
  mkdir(dir);
#else
  mkdir(dir, 0700);
  ...
  char *out             = dynprintf("%s%s%s%s%s", appdata, DIR_SEP, "crystaleer", DIR_SEP, "stdout.txt");
  g_log_stdout_filename = std::string(out);
  g_log_stdout          = fopen(out, "w+");

  char *err             = dynprintf("%s%s%s%s%s", appdata, DIR_SEP, "crystaleer", DIR_SEP, "stderr.txt");
